...
class StockAnalysisPipeline:
            ...
            try:
                ...
                if context and 'raw_data' in context:
                    ...
                    if isinstance(raw_data, list) and len(raw_data) > 0:
                        ...
                        # ✅ 传入 news_context，启用第四层舆情过滤
                        trend_result = self.trend_analyzer.analyze(df, code, news_context=news_context)
                        logger.info(f"[{code}] 趋势分析: {trend_result.trend_status.value}, "
                                  f"买入信号={trend_result.buy_signal.value}, 评分={trend_result.signal_score}")
                        if trend_result.sentiment_check:
                            logger.info(f"[{code}] 舆情过滤: {trend_result.sentiment_result}, "
                                      f"舆情评分={trend_result.sentiment_score}")
            except Exception as e:
    ...
    def _enhance_context(
        ...
        # 添加趋势分析结果（Pro 版 v2.0：包含三层决策和技术指标）
        if trend_result:
            enhanced['trend_analysis'] = {
                # 基础信息
                'market_type': trend_result.market_type,
                ...
                'three_layer_decision': {
                    'layer1_trend': '✅ 通过' if '多头' in trend_result.ma_alignment or 'MA5 > MA10 > MA20' in trend_result.ma_alignment else '❌ 未通过',
                    'layer1_detail': trend_result.ma_alignment,
                    'layer2_position': f"乖离率 {trend_result.bias_ma5:.2f}%",
                    'layer2_threshold': '5%' if trend_result.market_type == 'A股' else '6%',
                    'layer2_result': '✅ 通过' if abs(trend_result.bias_ma5) < (5.0 if trend_result.market_type == 'A股' else 6.0) else '❌ 未通过（追高风险）',
                    'layer3_base_score': 70,
                    'layer3_macd': '+10' if trend_result.macd_golden_cross else '+0',
                    'layer3_rsi': f"+{15 if trend_result.rsi < 30 else 10 if 30 <= trend_result.rsi <= 70 else 0}",
                    'layer3_atr': '+5' if trend_result.atr_pct < (3.0 if trend_result.market_type == 'A股' else 4.0) else '+0',
                ...
                'technical_indicators': {
                    'macd': {
                        'value': trend_result.macd,
                        'signal': trend_result.macd_signal,
                        'histogram': trend_result.macd_hist,
                        'golden_cross': trend_result.macd_golden_cross,
                        'status': '金叉' if trend_result.macd_golden_cross else '死叉',
                    },
                    'rsi': {
                        'value': trend_result.rsi,
                        'zone': '超买(>70)' if trend_result.rsi > 70 else '强势(50-70)' if trend_result.rsi >= 50 else '弱势(30-50)' if trend_result.rsi >= 30 else '超卖(<30)',
                        'status': '健康' if 30 <= trend_result.rsi <= 70 else '警惕',
                    },
                    'atr': {
                        'value': trend_result.atr,
                        'percent': trend_result.atr_pct,
                        'health': '健康' if trend_result.atr_pct < (3.0 if trend_result.market_type == 'A股' else 4.0) else '偏高',
                        'volatility': '低' if trend_result.atr_pct < 1.5 else '中' if trend_result.atr_pct < 3.0 else '高',
                    },
                },

                # Pro 版 v2.1 新增：第四层舆情过滤
                'fourth_layer_sentiment': {
                    'check_performed': trend_result.sentiment_check,
                    'result': trend_result.sentiment_result,
                    'score': trend_result.sentiment_score,
                    'reasons': trend_result.sentiment_reasons,
                } if trend_result.sentiment_check else None,
